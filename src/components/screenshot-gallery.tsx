...
export function ScreenshotGallery({
  ...
  return (
    <section
      ...
      aria-label={`${name} のスクリーンショット`}
      aria-roledescription="カルーセル"
    >
      <div className="gallery-heading-row">
        <div>
          <span className="section-index">IN THE APP</span>
          <h2>使う場面を、見てみる。</h2>
        </div>
        <p>横にスワイプ、または矢印で切り替え</p>
      </div>
      <div
        ref={viewport}
        ...
        tabIndex={0}
        role="group"
        aria-label="スクリーンショット。左右キーで切り替え"
      ...
      <div className="screen-controls">
        ...
        <div className="screen-dots" aria-label="表示する画面を選択">
          {images.map((shot, index) => (
            <button
              key={shot.src}
              type="button"
              aria-label={`${index + 1}枚目：${shot.caption}`}
